...
**SKN 텔레콤 — 통신사 고객 이탈 예측 및 고객 대응 추천**

### 팀원 및 역할

| 이름 | GitHub | 담당 역할 | 주요 작업                                              |
| --- | --- | --- |----------------------------------------------------|
| 안정민 | [@AhnJung-min](https://github.com/AhnJung-min) | PM · 서비스 통합 | 프로젝트 구조 설계, Streamlit 4탭, Test 평가, README·발표자료     |
| 이양원 | [@t91004](https://github.com/t91004) | 데이터 전처리 · EDA | 노트북 00·01, 데이터 품질 점검, EDA 7종, 전처리 스크립트, 전처리 결과서 작성 |
...
> 통신사 '고객관리팀 담당자'가 '이탈 가능성이 높은 고객을 사전에 식별'할 수 있도록 '고객의 계약 정보와 서비스 이용 데이터'를 이용해 '이탈 가능성이 높은 고객'을 예측하고, '고객 맞춤 유지 활동'을 제안한다.
...
| ID | 기능 | 설명 | 완료 여부 |
| --- | --- | --- | --- |
| FR-01 | 고객 현황 | 전체 고객의 유지/이탈 분포와 위험도 구간별 고객 수를 시각화합니다. | ✅ 완료 |
| FR-02 | 고객 조회 | 예시 고객(고위험·중위험·저위험)을 불러와 입력값으로 사용합니다. | ✅ 완료 |
| FR-03 | 개별 이탈 예측 | 고객 정보를 입력해 이탈 확률과 판정 결과를 출력합니다. | ✅ 완료 |
| FR-04 | 위험 등급 | 예측 확률에 따라 저위험·중위험·고위험 3단계로 표시합니다. | ✅ 완료 |
| FR-05 | 모델 성능 | 5개 모델의 동일 조건 비교와 최종 선정 근거, 혼동행렬을 보여줍니다. | ✅ 완료 |
...
| FR-07 | 유지 활동 제안 | 위험 등급에 맞는 유지 활동을 함께 제안합니다. | ✅ 완료 |
| FR-08 | 고객 세그먼트 | 신규 고객의 이용 지표로 5개 고객군 중 하나를 배정하고 전략을 제안합니다. | ✅ 완료 |
...
| 라이선스 | **Unknown** (Kaggle 표기) — 아래 유의사항 참고 |
| 수집 방법 | Kaggle 다운로드 |
| 수집·다운로드 일자 | 2026.07.14 |
| 데이터 규모 | 51,047행 × 58열 (학습용) / 20,000행 (holdout, Target 전부 결측) |

> **⚠️ 라이선스 유의사항**
> 이 데이터셋은 Kaggle에 라이선스가 **`Unknown`으로 표기**되어 있어, 재배포·상업적 이용이 허용되는지 확인할 수 없습니다.
> 따라서 다음 원칙을 지켰습니다.
>
> - **원본 CSV를 저장소에 포함하지 않습니다.** `.gitignore`로 `data/raw/`를 제외하고, 다운로드 방법만 안내합니다.
> - 이 프로젝트는 **비상업적 학습·교육 목적**으로만 데이터를 사용했습니다.
> - 원본은 2002년 Duke University **Teradata Center for CRM**이 churn 예측 대회용으로 공개한 데이터로, 다수의 학술 논문에서 인용되어 왔습니다.
>
> 아래 [설치 및 실행](#10-설치-및-실행)의 안내대로 직접 내려받아 `data/raw/`에 배치해 주세요.
...
- 전체 고객의 유지/이탈 분포와 규모
- EDA 핵심 인사이트와 해석 캡션
- 위험도 구간별 고객 수 집계 (Validation 기준, 저위험·중위험·고위험)
...
- 잘한 점: Streamlit으로 원본 데이터 현황, 최종 선정 모델의 성능 지표와 예측 결과를 확인할 수 있는 탭을 구현해 시각화를 체계적으로 구성했다. 또한 Git 관리를 담당하며 변경 사항을 공유하고 코드 충돌을 예방·해결해, 팀 프로젝트가 원활히 진행되도록 기여했다.
- 어려웠던 점: Git을 사용하며 항상 최신 버전을 유지하고, 작은 수정도 커밋과 푸시를 통해 팀원에게 공유하는 협업 방식이 처음에는 익숙하지 않았다. 병합 과정에서 발생한 충돌을 해결하는 과정에서도 여러 시행착오를 겪었다.
- 배운 점: 팀 협업에서는 각자가 맡은 역할을 책임감 있게 수행하는 것이 프로젝트의 안정적인 진행에 중요하다는 점을 배웠다. 또한 팀원마다 프로젝트를 바라보는 관점이 다를 수 있으므로, 피드백을 주고받는 과정이 결과물을 더 발전시킨다는 점을 깨달았다.
- 다음에 개선할 점: 다음 프로젝트에서는 초기에 목표·범위·역할 분담·일정 기준을 더 구체적으로 합의하고, Git 브랜치 및 PR 규칙을 미리 정해 관리하고자 한다. 이를 통해 변경 사항을 더 투명하게 공유하고, 충돌과 일정 지연을 줄이겠다.

### 이양원

- 잘한 점: 데이터 품질 점검과 EDA 결과를 실제 전처리 방향으로 연결하고, 전처리 스크립트와 결과서를 작성해 같은 과정을 재현할 수 있도록 했다.
- 어려웠던 점: 음수값과 특수 코드의 의미가 명확하지 않았고, 리텐션 변수는 접촉 시점이 없어 정보 누수 여부를 판단하기 어려웠다. 같은 형태의 컬럼이어도 각기 다른 전처리 방법을 이용해야 해서 어려움이 있었다.
- 배운 점: EDA에서 발견한 내용을 나열하는 것보다, 발견한 근거를 전처리와 모델링 의사결정으로 연결하는 것이 중요하다는 점을 배웠다.
- 다음에 개선할 점: 프로젝트 초기에 데이터 사전과 품질 검증 기준을 더욱 구체적으로 정의하고, 결측치·이상값·범주 코드 검사를 자동화하여 반복적인 확인 시간을 줄이고 싶다.

### 김유진

- 잘한 점:
  - 초반에 그라운드 설정 및 팀원 간 역할 분담이 잘 되었고 의견을 존중하고, 함께 프로젝트의 목표를 설정하며 프로젝트를 끝까지 계획대로 진행하였다.
  - 각자 맡은 역할뿐 아니라 모두가 새로운 내용을 배울 수 있도록 배려하며 지식을 공유해 준 점이 인상 깊었다. 결과물을 하나씩 완성해 가는 과정 자체가 즐거웠고, 배려와 협력의 중요성을 다시 한번 느낄 수 있었다.
  - 데이터 전처리부터 모델 학습, 성능 평가, 고객 군집화까지 팀에서 진행한 전체 머신러닝 프로젝트 흐름을 이해할 수 있었다.
  - Git 브랜치를 활용하여 협업하고 코드와 결과물을 공유하며 프로젝트를 완성하였다.
  - 단순히 높은 정확도를 목표로 하기보다 고객 이탈 예측의 목적에 맞게 Recall, F1-score, PR-AUC 등 다양한 성능 지표를 비교하며 모델을 선정하였다.
  - 모델의 성능뿐 아니라 비즈니스 관점의 활용 방안으로까지 연결하여 보다 실무적으로 접근할 수 있었다.

- 어려웠던 점:
  - data의 feature 용어들이 비슷한 것들이 많아서 단어 사전을 만들어 일부 이해가 필요하였다.
  - 표나 그래프 등 시각화 자료가 처음에는 낯설어 눈에 익히고 해석하는 노력이 필요하였다. 결과적으로 시각화자료를 보는 눈이 길러졌다.
  - 맡은 부분 외에도 전처리, 클러스터링까지 다른 팀원이 진행한 부분도 이해하는 과정이 시간과 에너지가 들고 쉽지 않았으나 필요한 과정이었고 이해하고 진행하는 재미와 뿌듯함이 있었다. 

- 배운 점:
  - Accuracy만으로 모델을 평가하는 것이 아니라 프로젝트 목적에 맞는 평가 지표를 선택하는 것이 중요하다는 점을 이해했다.
  - 머신러닝 프로젝트를 진행하며 담당한 모델링 과정도 신기하고 재미있었는데 팀원들과 데이터 분석, 결과 해석, 발표 준비까지 잘 진행되어 더 좋았다. 그럴 수 있었던 것은 서로를 이해하는 소통과 배려였다고 생각한다.
  - Git commit 과정에서 한 번 충돌이 있었는데 파이참에서 어느쪽을 반영할 것인지 선택하는 창이 뜨는 것을 알게 되었고 직접 검토하며 반영해볼 수 있었다.
  - 수업에서 배운 모델별 특성을 더 깊이 이해할 수 있는 시간이었다.

- 다음에 개선할 점: 
  - 모델 성능을 더 향상시킬 수 있는 Feature Engineering이 있을지 알아보고 싶다.

### 김성훈

- 잘한 점: 이탈 예측에 그치지 않고, 고객 세그먼트별 맞춤 유지 전략까지 제안해 분석 결과를 실제 활용 방안으로 연결했다.
- 어려웠던 점: K-Means의 k를 정할 때 지표뿐 아니라 군집의 해석 가능성과 실무 활용성을 함께 고려해야 했다.
- 배운 점: 성능이 높은 모델보다 데이터 누수 가능성, 운영 가능성, 평가 지표의 의미를 함께 고려하는 것이 중요하다는 점을 배웠다.
- 다음에 개선할 점: 실제 캠페인 비용과 유지 효과를 반영해 임계값과 혜택 전략을 검증하고, 운영 데이터로 모델을 지속적으로 개선하고 싶다.
...
- [x] 데이터 출처, URL, 라이선스를 작성했습니다. *(라이선스 `Unknown` — 원본 미포함으로 대응)*
...
- [x] 새 환경에서 README 순서대로 실행되는지 확인했습니다.
- [x] 대용량 데이터, 가상환경, 비밀번호, API Key를 커밋하지 않았습니다.
- [x] 수행 화면 캡처를 `assets/images/screenshots/`에 추가했습니다.