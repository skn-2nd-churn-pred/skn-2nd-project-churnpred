...
| 우선 지표 | ROC-AUC 우선 확인, F1·PR-AUC 병행 (FN 비용 > FP 비용) |
...
가장 중요한 이슈는 **리텐션 컬럼 3개의 역인과관계 의심**이다(6-2절). 이 컬럼들을 포함하면 성능이 인위적으로 높아 보일 수 있어, 포함/제외 두 버전을 모두 만들어 비교 실험을 할 것이다. 이 결정이 이번 전처리·EDA 단계에서 가장 큰 판단이었고, 최종 채택 여부와 근거는 `modeling_report.md`에 기록될 것이다.
...
| EDA 시각화 7종 + 인사이트 3개 + 결정 3개                  | ✅ 완료 | `notebooks/01_eda.ipynb`, `reports/figures/*.png`                           |
...
두 버전(with/without_retention)의 Validation 성능(ROC-AUC·F1·PR-AUC)을 비교해서 하나를 최종 채택하고, 그 근거를 `modeling_report.md`에 남기면 이 프로젝트의 데이터 준비 단계는 완전히 마무리된다.
