...
| 파일 | `cell2celltrain.csv`(51,047명, Target 포함) / `cell2cellholdout.csv`(20,000명, Target 전부 결측) |
...
`cell2cellholdout.csv`는 `Churn` 컬럼이 20,000행 전부 결측이라 성능 평가에 쓸 수 없다. 이 파일은 **Test로 사용하지 않고, Streamlit "신규 고객 일괄 예측" 시연 전용**으로만 남겨둔다.
...
![equipment days](figures/02_equipmentdays_by_churn.png)
...
![retention call churn rate](figures/03_retentioncall_churn_rate.png)
...
![web capable churn rate](figures/04_webcapable_churn_rate.png)
...
![credit rating churn rate](figures/05_creditrating_churn_rate.png)
...
![region churn rate](figures/06_region_churn_rate.png)
...
![numeric correlation](figures/07_numeric_corr_with_target.png)
...
### 6-2. 리텐션 관련 3개 컬럼 → 포함/제외 두 버전 모두 생성
③번 관찰에서 발견한 역인과관계 가능성 때문에, 이 컬럼을 넣을지 뺄지를 전처리 단계에서 미리 결정하지 않고 `train_with_retention.csv` / `train_without_retention.csv` 두 세트를 모두 만들었다. 모델링 담당자가 두 버전의 성능·Feature Importance를 비교해서 최종 채택 여부와 근거를 학습 결과서에 남기기로 했다.
...
`stratify=Churn`을 적용한 랜덤 분리이며(고객 1명=1행, 시간/그룹 분할 불필요), ServiceArea 희소 지역 그룹화·결측치 대표값·인코딩 범주·스케일 모두 이 분리 이후 Train에서만 계산했다. `cell2cellholdout.csv`는 이 분리에 전혀 관여하지 않는다.

## 8. Target 누수(Leakage) 점검

- 해지 사유, 해지일 등 이탈 이후에만 생기는 정보에 해당하는 컬럼은 없음을 확인했다.
- **리텐션 컬럼 3개는 완전히 배제하지 않고 "의심 표시 + 두 버전 비교"로 처리**했다 (6-2절). 성능이 지나치게 좋아지는 방향으로 이 컬럼들이 기여한다면, 최종 모델에서는 제외를 권장할 것이다.

## 9. 산출물

```
data/raw/cell2celltrain.csv                       원본 (수정 없음, 51,047행)
data/raw/cell2cellholdout.csv                      원본 (수정 없음, Streamlit 시연 전용)
...
data/processed/                                    아직 비어 있음 — 최종 버전을 확정하면 채워짐
...
notebooks/01_data_check.ipynb                      1차 데이터 점검 (shape·중복·결측·지저분한 컬럼 미리보기)
notebooks/02_eda.ipynb                              EDA 시각화 7종 + 인사이트·결정·한계 (실행 결과 내장)
reports/figures/*.png                               EDA 시각화 7종 (마크다운용 별도 파일)
...
가장 중요한 이슈는 **리텐션 컬럼 3개의 역인과관계 의심**이다(6-2절). 이 컬럼들을 포함하면 성능이 인위적으로 높아 보일 수 있어, 포함/제외 두 버전을 모두 만들어 비교 실험을 할 것이다. 이 결정이 이번 전처리·EDA 단계에서 가장 큰 판단이었고, 최종 채택 여부와 근거는 `training_report.md`에 기록될 것이다.
...
| 결과물                                           | 상태 | 위치                                                                          |
|-----------------------------------------------|---|-----------------------------------------------------------------------------|
| 데이터 품질 점검                                     | ✅ 완료 | `notebooks/01_data_check.ipynb`                                             |
| EDA 시각화 7종 + 인사이트 3개 + 결정 3개                  | ✅ 완료 | `notebooks/02_eda.ipynb`, `reports/figures/*.png`                           |
| 전처리 Pipeline (리텐션 포함/제외 두 버전)                 | ✅ 완료 | `src/data.py` → `artifacts/preprocessor_*.joblib`                           |
| Train/Validation/Test 분리 (60/20/20, stratify) | ✅ 완료 | `data/interim/*.csv`                                                        |
| Feature 메타데이터                                 | ✅ 완료 | `artifacts/feature_schema.json`                                             |
| 전처리 결과서(본 문서)                                 | ✅ 완료 | `reports/preprocessing_report.md`                                           |
| 학습 테이블 생성                                     | ✅ 완료 | `data/interim/{train,val,test}_with_retention.csv 와 _without_retention.csv` |

두 버전(with/without_retention)의 Validation 성능(Recall·F1·PR-AUC)을 비교해서 하나를 최종 채택하고, 그 근거를 `training_report.md`에 남기면 이 프로젝트의 데이터 준비 단계는 완전히 마무리된다.
